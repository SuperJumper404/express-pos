...
assert.match(migration, /NULLIF\(CAST\(`updated` AS CHAR\), '0000-00-00 00:00:00'\)/);
assert.match(migration, /NULLIF\(CAST\(`created` AS CHAR\), '0000-00-00 00:00:00'\)/);
assert.match(migration, /COALESCE\([\s\S]*`updated`[\s\S]*`created`[\s\S]*NOW\(\)[\s\S]*\)/);
