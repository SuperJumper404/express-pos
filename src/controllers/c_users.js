...
module.exports = {
  register: (req, res) => {
    const body = req.body
    console.log("NEw Registering process",req.body)
  ...
  login: (req, res) => {
    ...
    mCheckEmail(body.email).then( async(response) => {
      ...
      if (response.length === 1) {
        ...
        // If we have more issue with login We need to review token acces management
        console.log("Check pass",checkPass,"body",body.password , "res", response[0].clearpass)
        if (checkPass) {
