...
module.exports = {
  ...
  mDetailProduct: (id) => {
    return new Promise((resolve, reject) => {
      conn.query('SELECT *, products.id AS id, products.name AS name, products.description AS description,  category.name AS category FROM products LEFT JOIN category ON products.categoryId=category.id WHERE products.id = ?', [id], (err, result) => {
        if (!err) {
          resolve(result)
        } else {
          reject(new Error(err))
